// The CSS named colors, fixed by spec. Kept whole rather than curated so
// there is no cutoff to justify: "rebeccapurple" resolves like "white" does.
...
function hexToRgb(hex) {
  const intColor = Number.parseInt(hex, 16);
  return {
    r: (intColor >> 16) & 255,
    g: (intColor >> 8) & 255,
    b: intColor & 255,
...
function parseHexColor(color) {
  ...
  return /^[0-9a-f]{6}$/i.test(clean) ? hexToRgb(clean) : null;
}
...
function parseNamedColor(color) {
  const hex = CSS_COLOR_NAMES[color];
  return hex ? hexToRgb(hex) : null;
}

// The single place that decides what counts as a color. Returns {r,g,b} or
// null; alpha is accepted syntactically and then dropped, since the card
// derives an opaque accent of its own.
...
export function getChroma({ r, g, b }) {
  return (Math.max(r, g, b) - Math.min(r, g, b)) / 255;
}

export function isChromatic(rgb) {
  return getChroma(rgb) >= MIN_CHROMA;
}
